...
llm = ERNIEBot(
    model="ernie-bot", api_type="aistudio", access_token="7d109d14c26a3e0e5a01f841927c30331ad07e62"
)
toolkit = CVToolkit()
memory = WholeMemory()
...
async def run_agent():
    seg_file = await file_manager.create_file_from_path(
        file_path="/Users/tanzhehao/Downloads/43.png", file_type="local"
    )
    ...
    # print(agent_resp.annotations)
    # print("\n" + "=" * 20 + "\n")
    agent_resp = await agent.async_run("请帮我将这张图中的海分割出来", files=[seg_file])
    print("=" * 10 + "分割返回结果" + "=" * 10 + "\n")
    print(agent_resp.text)
    print(agent_resp.annotations)
    print("\n" + "=" * 20 + "\n")
